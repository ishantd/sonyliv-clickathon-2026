import Image from "next/image";

/**
 * ClickHouse's symbol — the bars — in its own colours.
 *
 * The exact 9×8 geometry from the official asset, which is the same path the
 * submission deck draws (pitch/index.html). Bringing it here is what makes the
 * product and the deck read as one submission rather than two designs: the deck's
 * cover is this mark beside SonyLIV's, and the dashboard's header now is too.
 *
 * IN COLOUR, which is a deliberate exception to this app's one-signal-colour
 * rule and the only one. Everywhere else, hue is reserved for state — gold means
 * "SonyLIV" or "happening now", red means "wrong". A logo is neither: it is
 * someone else's property, and the rule that protects the interface from
 * inventing hues is not a licence to restyle a mark that is not ours. The red
 * pixel in particular is the thing that distinguishes ClickHouse's mark from a
 * bar chart, and it is 1/72nd of a 16px glyph — too small to compete with
 * anything the interface is trying to say.
 */
const CH_YELLOW = "#FFCC00";
const CH_RED = "#FF0000";

export function ClickHouseSymbol({ className = "h-4 w-auto" }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 9 8"
      className={className}
      role="img"
      aria-label="ClickHouse"
    >
      <path d="m0 7h1v1h-1z" fill={CH_RED} />
      <path
        d="m0 0h1v7h-1zm2 0h1v8h-1zm2 0h1v8h-1zm2 0h1v8h-1zm2 3.25h1v1.5h-1z"
        fill={CH_YELLOW}
      />
    </svg>
  );
}

/**
 * ClickHouse's wordmark, verbatim from clickhouse.com.
 *
 * Their own SVG, not a redraw: it ships fill="currentColor", so the lockup sets
 * the colour and the mark inherits it. Redrawing someone's logo by eye is how you
 * end up shipping a subtly wrong version of it.
 */
export function ClickHouseMark({ className = "h-4 w-auto" }: { className?: string } = {}) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 135 40" fill="currentColor" role="img"
      className={className} aria-label="ClickHouse" ><rect width="2.25" height="20.25" x="2.71" y="9.88" rx="0.24"></rect><rect width="2.25" height="20.25" x="7.21" y="9.88" rx="0.24"></rect><rect width="2.25" height="20.25" x="11.71" y="9.88" rx="0.24"></rect><rect width="2.25" height="20.25" x="16.21" y="9.88" rx="0.24"></rect><rect width="2.25" height="4.5" x="20.71" y="17.75" rx="0.24"></rect><path d="M40.03 15.14q-.95 0-1.7.34-.76.33-1.3.98-.52.64-.81 1.56-.27.91-.27 2.07a7 7 0 0 0 .45 2.63q.45 1.1 1.35 1.7t2.27.59q.83 0 1.58-.15.78-.15 1.57-.41v1.67q-.75.3-1.55.42-.8.15-1.84.14-1.96 0-3.27-.81a5 5 0 0 1-1.95-2.3q-.65-1.5-.65-3.5 0-1.46.4-2.66.42-1.23 1.19-2.1a5 5 0 0 1 1.9-1.36 7 7 0 0 1 2.65-.48 8.5 8.5 0 0 1 3.6.79l-.72 1.62q-.62-.3-1.37-.5a5 5 0 0 0-1.53-.24m7.6 11.36h-1.91V12.82h1.9zm4.9-9.7v9.7h-1.9v-9.7zm-.94-3.7q.44.01.76.26t.32.85q0 .57-.32.84a1.2 1.2 0 0 1-.76.25q-.45 0-.79-.25-.3-.27-.3-.84 0-.6.3-.85.33-.25.8-.25m7.84 13.58q-1.34 0-2.34-.52a3.6 3.6 0 0 1-1.56-1.62 6 6 0 0 1-.56-2.83q0-1.8.6-2.91.6-1.13 1.63-1.64a5 5 0 0 1 2.38-.54q.81 0 1.5.18.73.15 1.2.38l-.58 1.54q-.51-.2-1.08-.34-.56-.15-1.06-.14-.9 0-1.5.4-.57.37-.86 1.15-.27.76-.27 1.9 0 1.1.29 1.86.3.75.84 1.15.58.38 1.43.38a5 5 0 0 0 2.57-.65v1.66q-.53.3-1.13.45t-1.5.14m6.81-7.02q0 .38-.04.86-.01.5-.05.9h.05l.78-.97q.2-.26.4-.47l2.96-3.18h2.22l-3.9 4.16 4.15 5.54h-2.25l-3.2-4.34-1.12.94v3.4h-1.89V12.82h1.9zm18.4 6.84H82.7v-5.87h-6.13v5.87h-1.95V13.65h1.95v5.33h6.13v-5.33h1.95zm11.78-4.86q0 1.2-.32 2.14a5 5 0 0 1-.92 1.59q-.6.65-1.44.99a5.3 5.3 0 0 1-3.71 0 4.1 4.1 0 0 1-2.38-2.58 6 6 0 0 1-.34-2.16q0-1.6.54-2.72.56-1.1 1.59-1.69 1.04-.6 2.44-.6 1.34 0 2.34.6 1.03.58 1.6 1.7.6 1.11.6 2.73m-7.15 0q0 1.08.27 1.87.28.78.85 1.19t1.48.41q.9 0 1.47-.41.58-.42.85-1.19.27-.8.27-1.87 0-1.11-.29-1.87-.27-.76-.85-1.15a2.4 2.4 0 0 0-1.47-.42q-1.36 0-1.96.9-.62.9-.62 2.54m17.92-4.84v9.7h-1.53l-.27-1.28h-.09q-.3.5-.8.83-.47.33-1.05.47-.58.16-1.2.16-1.13 0-1.92-.36a2.6 2.6 0 0 1-1.18-1.15 4.5 4.5 0 0 1-.4-2.02V16.8h1.92v6.06q0 1.14.47 1.7.5.55 1.5.55t1.58-.4q.59-.39.81-1.14.25-.78.25-1.86V16.8zm9.43 6.96q0 .96-.47 1.6a3 3 0 0 1-1.35 1q-.88.32-2.12.32-1.03 0-1.77-.16-.71-.15-1.33-.43v-1.7q.65.31 1.5.56a6 6 0 0 0 1.65.23q1.08 0 1.55-.34.5-.34.49-.92 0-.31-.18-.57a2 2 0 0 0-.69-.54q-.48-.3-1.44-.65a15 15 0 0 1-1.56-.74 3 3 0 0 1-1-.88q-.34-.53-.34-1.33 0-1.26 1.01-1.93a5 5 0 0 1 2.7-.68 7 7 0 0 1 3.19.68l-.63 1.46a7 7 0 0 0-1.75-.56 4 4 0 0 0-.9-.09q-.86 0-1.31.27a.8.8 0 0 0-.45.76q0 .34.2.6.21.24.73.5.53.25 1.43.6t1.53.71q.64.36.97.88.34.53.34 1.33m6.1-7.14q1.27 0 2.19.54.92.52 1.4 1.51.5.99.5 2.34v1.04h-6.51q.03 1.5.77 2.29.75.8 2.11.8a8 8 0 0 0 1.66-.17q.74-.19 1.5-.52v1.58a7 7 0 0 1-3.23.65q-1.41 0-2.49-.56a4 4 0 0 1-1.69-1.65q-.6-1.12-.6-2.74 0-1.65.55-2.77a4.1 4.1 0 0 1 3.83-2.34m0 1.47q-1.04 0-1.66.67-.62.66-.72 1.89h4.57q0-.76-.24-1.33-.23-.59-.72-.9a2.2 2.2 0 0 0-1.24-.33"></path></svg>
  );
}

/**
 * SonyLIV's "liv" mark, from their own header asset.
 *
 * A PNG because that is what they serve — the site has no SVG version of it. Kept
 * in public/ rather than base64'd into the bundle: the static export copies it to
 * out/, which the Go binary already embeds and serves, so it stays cacheable and
 * out of the JavaScript.
 *
 * unoptimized is set globally in next.config (there is no image pipeline behind a
 * Go binary), so this renders the file as-is at ~4x the displayed height.
 */
/*
 * basePath is NOT applied to this src automatically, which is worth stating because
 * it contradicts the reasonable expectation. Next rewrites the URLs it generates —
 * links, chunks, the /_next tree — but with images: { unoptimized: true } the src
 * passes through verbatim, so under NEXT_PUBLIC_BASE_PATH=/build the emitted
 * preload came out as /sonyliv-mark.png and 404'd against nginx, which routes only
 * /build/ here. Verified in dashboard/out/index.html before this line existed.
 *
 * Prefixed explicitly rather than by dropping `unoptimized`: there is no image
 * pipeline behind a Go binary to optimize with.
 */
const assetBase = process.env.NEXT_PUBLIC_BASE_PATH ?? "";

export function SonyLivMark() {
  return (
    <Image
      src={`${assetBase}/sonyliv-mark.png`}
      alt=""
      width={96}
      height={96}
      priority
      className="h-[1.375rem] w-auto"
    />
  );
}
