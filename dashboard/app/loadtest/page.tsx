...
import { useWriteDataset } from "@/lib/dataset";
...
export default function LoadSimulator() {
  ...
  //
  // PINNED TO THE WRITE TARGET, for the reason /live was pinned in 5c892dd: this
  // curve is about the load THIS page is generating, so charting it against a
  // dataset this page cannot write to is a category error, and one that reads as
  // a broken pipeline rather than as a mismatch.
  //
  // But note the asymmetry with /live, because it is why this sends a `db` rather
  // than sending nothing. /live's handler pins server-side and ignores the
  // parameter, so omitting it there is exact. /api/curve still resolves an empty
  // `db` through resolveDatabase, whose empty-string case is the READ default —
  // `sonyliv`, the frozen 31 Jul extract. Sending nothing here would therefore
  // chart the last 30 minutes of a dataset whose events stopped days ago: a flat
  // zero line while the generator is visibly running. Naming the writable
  // database explicitly is what actually reaches the rows this page creates.
  //
  // Held null until the dataset list resolves, rather than firing an unqualified
  // request first: one poll against the wrong database is one frame of a curve
  // that is not this one.
  const writeDb = useWriteDataset();
  const { data: curve } = useSWR<CurveResponse>(
    writeDb ? `/api/curve?minutes=30&db=${writeDb.name}` : null,
    fetcher,
