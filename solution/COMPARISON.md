...
- daily reset/splitting for bounded prefix sums;
...
| Foreground | Foreground or a later qualifying event reactivates. | Foreground only changes visibility; it cannot undo pause or renew an expired playback lease. | 13,497/14,321 Foreground markers retain pause as latest playback state. Treating FG as liveness adds 0.546768h across 152 sessions at TTL120. Keep the conservative independent rule unless judges say otherwise. |
| Time fidelity | Activity is collapsed to distinct minutes and expanded to minute runs (`groupUniqArray(minute)`). | State is evaluated at exact `DateTime64(3)` boundaries; minute statistics are derived afterward. | Exact hot-hour in-minute peak is 2,305, while exact minute-boundary sample peak is 2,285. Minute collapse cannot answer both semantics. |
...
| Terminal End | Last End wins (`max`/`argMax`). | First End is terminal; later input is quarantined. | Four sessions have different End times; 241 sessions have 870 rows after first End. A terminal event should not reopen merely because anomalous tail rows exist. Confirm with judges if they define otherwise. |
...
3. the signed old/new boundary-map path in `20_publish_boundaries.sql`;
