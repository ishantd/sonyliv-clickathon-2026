...
|---|---|---|
| Live demo | `sonyliv_demo` | Created empty. Everything in it was generated by the simulator, so the curve starts at zero and builds while you watch. **Writable — the only one.** |
| Tuning extract — 14–26 Jul | `sonyliv_prod` | The graded July extract plus the load tests run during the build. The hot hour reproduces 2,305 / 855.578199. **Read-only.** |
| Evaluation set — 31 Jul | `sonyliv_unseen` | The sealed unseen day, exactly as loaded. **Read-only.** |

The picker only offers datasets that exist on the connected server, so the same binary
shows three options against Cloud and one against the single-node ClickHouse that
`docker compose up` brings up — rather than offering two that error.
...
> ### ⚠️ Known defect — 14,506 is an undercount
>
...
>
> **The interval builder silently drops every session that has no `VideoSessionStart`.**
...
>
> Not zero-length sessions, and not noise: the 25,403 average **103.88 events each** and
> carry `liveness`, `pause`, `resume`, `background` and `foreground` signals. They are real
> viewing. They are sessions already in flight when the extract's window opens — the
> boundary case the sibling pipeline handles with an explicit `allow_boundary_sessions`
> parameter, and that this one does not handle at all.
>
> This was first read as an unfinished build, because `dirty_sessions` still held all
> 108,486 keys. It is not. Loading the same 7,000,000 events into a **brand-new
> single-node ClickHouse** via `docker compose up` reproduces 83,083 interval rows,
> 123,732 intervals and a peak of 14,506 at 11:15 — bit for bit. A deterministic result on
> fresh infrastructure is a rule, not a stalled queue.
>
> So `14,506` is what our pipeline currently produces and `19,882` is the better number.
> The fix is to open an interval at `session_start_epoch` (or at the first foreground
> signal) when no `VideoSessionStart` is present, rather than declining to emit a row.
>
> This is written down rather than rounded off because a silently wrong concurrency figure
> is the precise failure mode this problem statement exists to punish — and this one is
> ours, found by cross-checking two implementations against each other.
...
```bash
git clone https://github.com/ishantd/sonyliv-clickathon-2026
cd sonyliv-clickathon-2026
...
That single command starts ClickHouse, creates the database, applies the full schema,
loads the extracts if you have them, starts the ingest API, the rollup loop and the
dashboard, and waits for each step before starting the next.
[`.env.example`](.env.example) documents every knob; the interesting ones are
`CH_DATABASE`, `DASHBOARD_PORT` and `DATA_DIR`.

### Loading the supplied extracts

Optional. Drop either or both CSVs into `./data` before `docker compose up`:
...
The unseen day's files work unchanged — the loader globs the prefix, so `*_surprise.csv`
is picked up with no config change. Measured on a laptop: **7,000,000 events in 1 m 10 s
at 100,295 rows/s**, 0 retries, then the rollup builds the whole serving layer — 17 days,
1,006,755 minute rows — in about 12 seconds.

With no CSVs the stack still comes up. That is a working demo rather than a degraded one:
create a fleet on `/fleet/new/` and the curve builds from the simulator's own traffic,
which is closer to what the product is actually for. `--profile traffic` starts one for
you.

If the catalogue is the only file you load, load that one. Without it every session
resolves to `__unknown__` and the title, category and content-type panels collapse to a
single bar.

### Everything else the compose file can do
...
The `mcp` profile is a functional demo of the tools, **not** of the security boundary: it
connects as the same user as everything else, whereas the real deployment connects as
`sonyliv_mcp`, which cannot read `events_clean` or `session_intervals` at all. Run
[`ingest/sql/manual/009_mcp_reader.sql`](ingest/sql/manual/009_mcp_reader.sql) against the
container to reproduce that.

LibreChat is not in this compose file. It needs an LLM key, and a chat window that cannot
reach a model is worse than an absent one. It has its own stack:
...
Against ClickHouse Cloud, which is what the hosted demo does — the compose stack is a
single node and cannot reproduce the Cloud behaviours in [`CLAUDE.md`](CLAUDE.md).
Needs Go ≥ 1.25 and, only if you change `dashboard/`, Node ≥ 20.9.

```bash
cp ingest/.env.example .env && $EDITOR .env   # host, port 9440, secure, user, password
make -C ingest schema                         # CREATE DATABASE first: schema makes tables
./scripts/bootstrap.sh --events path/to/raw.csv --content path/to/content.csv
make -C ingest rollup                         # or rollup-live for a 10s loop
make -C ingest rollup-check                   # gating: exits non-zero on any FAIL
make -C ingest api                            # :8080, serves the embedded dashboard
```

Verify correctness against something outside the pipeline:
...
| Path | What's in it |
|---|---|
| [`docker-compose.yml`](docker-compose.yml) · [`Dockerfile`](Dockerfile) | The whole stack in one command, ClickHouse included |
| [`deploy/docker/init.sh`](deploy/docker/init.sh) | What `docker compose up` runs first: database, schema, data, dictionary |
