...
var databases = []database{
	// First, because it is the one that MOVES. A demo opens here.
	//
	// Its windows are all relative and all short, and that is the point rather than
	// an omission: this database was created empty and holds only what the simulator
	// has produced since, so an absolute window would be an empty chart on any day
	// but the one it was written.
	{
		Name:     "sonyliv_demo",
		Label:    "Live demo",
		Writable: true,
		Note:     "Created empty. Everything here was generated by the simulator on this box, so the curve starts at zero and builds while you watch. The only dataset the simulator writes into.",
		Windows: []window{
			{Key: "15m", Label: "Last 15 minutes", RelMinutes: 15},
			{Key: "1h", Label: "Last hour", RelMinutes: 60},
			{Key: "6h", Label: "Last 6 hours", RelMinutes: 360},
			{Key: "24h", Label: "Last 24 hours", RelMinutes: 1440},
		},
	},
	// The graded July extract. Read-only now, not by a guard but by arithmetic: the
	// write paths use the server's configured database and that is sonyliv_demo, so
	// there is no request that reaches this one with an INSERT.
	//
	// Kept in the picker rather than retired, because it is the only database where
	// the canonical hot-hour figures reproduce — 2,305 peak, 855.578199 average —
	// and being able to show the number the whole design was verified against is
	// worth a menu entry.
	//
	// "Whole extract" is deliberately NOT the default window. This database also
	// carries every load test ever run against it, including a 100,000-session run
	// on 2026-08-01 that peaks at 146,918, so the full span renders the real traffic
	// as a flat line under one spike. The hot hour is the honest view of it.
	{
		Name:     "sonyliv_prod",
		Label:    "Tuning extract — 14–26 Jul",
		Writable: false,
		Note:     "The graded July extract plus the load tests run against it during the build. Read-only now that the simulator writes to the demo database. The hot hour reproduces the canonical 2,305 / 855.578199.",
		Windows: []window{
			...
			{Key: "loadtest", Label: "100k load test (1 Aug)", From: "2026-08-01 23:00:00", To: "2026-08-02 00:00:00"},
		},
	},
	{
		Name:     "sonyliv_unseen",
...
// ResolveDatasets narrows the selector to the datasets that actually exist.
//
// WHY. The list above is the Cloud deployment's three databases, written down in
// Go because the name is interpolated into panel SQL as a schema identifier and
// an allowlist is the only safe mechanism for that. But the same binary now runs
// against a single-node ClickHouse that docker-compose.yml brings up empty, where
// exactly one of those three exists. Left alone, the picker offered all three and
// two of them returned "Unknown database" from ClickHouse — a real error, from a
// control the UI itself rendered, which reads as the product being broken.
//
// The server's own configured database is always included, whether or not it is
// in the allowlist. That is not a hole: it is this process's own configuration,
// not request text, and it is already the fallback that every unqualified query
// uses. Without it, `CH_DATABASE=anything` would come up with an empty picker.
//
// Failure is non-fatal and leaves the full list in place. A dataset picker is not
// worth refusing to serve the dashboard over, and the pre-existing behaviour is
// the correct thing to fall back TO.
func (s *Server) ResolveDatasets(ctx context.Context) error {
	rows, err := s.client.Conn.Query(ctx, "SELECT name FROM system.databases")
	if err != nil {
	...
	out := make([]database, 0, len(databases))
	...
	if cur := s.client.Database; cur != "" && !seen[cur] {
		// Prepended, not appended: it is the default, so it is the one the page
		// opens on, and a picker whose selected entry is last in the list reads
		// as though something else was chosen for you.
		out = append([]database{{
			Name:     cur,
			Label:    cur,
			Writable: true,
			Note:     "This server's configured database.",
			Windows: []window{
				{Key: "15m", Label: "Last 15 minutes", RelMinutes: 15},
				{Key: "1h", Label: "Last hour", RelMinutes: 60},
				{Key: "6h", Label: "Last 6 hours", RelMinutes: 360},
...
// resolveDatabase maps the request's `db` to a selectable name.
//
...
func (s *Server) resolveDatabase(requested string) (string, error) {
	if requested == "" {
		return s.client.Database, nil
	}
